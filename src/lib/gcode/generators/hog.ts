...
function top_hat(xstart: number, ystart: number, xend: number, yend: number, height: number, brim: number, zup: string, zdn: string, rapid: number, vertical: number, drawspeed: number): string {
...
function top_hat_dbl(xstart: number, ystart: number, xend: number, yend: number, height: number, brim: number, zup: string, zdn: string, rapid: number, vertical: number, drawspeed: number, offs: number): string {
  const dx = xend - xstart
  const dy = yend - ystart
  const oal = Math.sqrt(dx * dx + dy * dy)
  const xu = dx / oal
...
function full_speed_swipe(xstart: number, ystart: number, xend: number, yend: number, zup: string, zdn: string, rapid: number, vertical: number, drawspeed: number): string {
  let out = ''
  out += '; full speed cut\n'
  out += 'G0' + zup + ' F' + vertical + '\n'
  out += 'G0 X' + xstart.toFixed(3) + ' Y' + ystart.toFixed(3) + ' F' + rapid + '\n'
...
function hog_test_x(cornerx: number, cornery: number, stepover: number, zup: string, zdn: string, rapid: number, vertical: number, drawspeed_slow: number, drawspeed: number): string {
  ...
  let out = ''
  out += top_hat_dbl(xstart, ystart, xend, yend, height, brim, zup, zdn, rapid, vertical, drawspeed_slow, stock_to_leave)
...
function hog_test_y(cornerx: number, cornery: number, stepover: number, zup: string, zdn: string, rapid: number, vertical: number, drawspeed_slow: number, drawspeed: number): string {
  const xstart = cornerx
...
export function generateHog(
  orientation: 'X' | 'Y',
  hog_count: number, hog_offset: number,
  final_feedrate: number, final_stepover: number,
  stepover: number,
  u: UniversalParams
): string {
  ...
  for (let i = 0; i < hog_count; i++) {
    if (i === 0) {
      ...
      out += '; feedrate ' + drawspeed + ' mm/min, stepover ' + stepover.toFixed(3) + ' mm, volumetric ' + Math.round(vol) + ' mm3 per sec\n'
    } else {
      const vol = -pen_d * (drawspeed + i * speed_step) * (stepover + i * stepover_step)
      out += '; feedrate ' + Math.round(drawspeed + i * speed_step) + ' mm/min, stepover ' + (stepover + i * stepover_step).toFixed(3) + ' mm, volumetric ' + Math.round(vol) + ' mm3 per sec\n'
    }
  }
  ...
  if (orientation === 'X') {
    for (let i = 0; i < hog_count; i++) {
      if (i === 0) {
        ...
        out += hog_test_x(0, i * hog_offset, stepover + i * stepover_step, zu, zd, rapid, vertical, drawspeed_slow, drawspeed + i * speed_step)
      }
    }
  } else {
    for (let i = 0; i < hog_count; i++) {
      if (i === 0) {
        ...
        out += hog_test_y(i * hog_offset, 0, stepover + i * stepover_step, zu, zd, rapid, vertical, drawspeed_slow, drawspeed + i * speed_step)
      }
    }
  }
